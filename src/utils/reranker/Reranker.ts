import {
  ...
  RerankerInput,
  RerankerOutput,
  WorkerResponseReranker,
} from "./types.ts";

class Reranker extends EventTarget {
  private worker: Worker;
  private logger: (data: any) => any;
  private queue: Array<{ id: number; input: RerankerInput }> = [];
  ...
  private workerMessage = (
    id: number,
    texts: RerankerInput,
    onMessage: (e: WorkerResponseReranker) => void
  ): Promise<RerankerOutput> =>
    new Promise((resolve, reject) => {
      this.worker.postMessage({ input: texts, id, log: false });
      const listener = (e: MessageEvent<WorkerResponseReranker>) => {
